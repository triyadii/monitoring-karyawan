...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var datatable = $('#kt_table_hos').DataTable({
        ajax: {
            url: isSuperAdminOrLeader ? `${apiUrl}/manajemen-ho` : `${apiUrl}/manajemen-ho/user/` + currentUserId,
            type: 'GET',
            headers: { 'Authorization': `Bearer ${token}` },
            data: function(d) {
                if (selectedUserId) {
...
window.viewMemberData = function(userId, el) {
    ...
    $('#kt_table_hos').DataTable().ajax.url(`${apiUrl}/manajemen-ho/user/${userId}`).load();
}
...
window.showAllMembersData = function(el) {
    ...
    $('.member-card').removeClass('border-primary bg-light-primary');
    if (el) $(el).addClass('border-primary bg-light-primary');
    $('#kt_table_hos').DataTable().ajax.url(`${apiUrl}/manajemen-ho`).load();
}
});
</script>
@endsection
