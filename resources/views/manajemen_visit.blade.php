            ...
            <div class="card card-flush">
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar" id="btn_add_container" style="display: none;">
                        <button type="button" class="btn btn-success me-3" onclick="exportData('visit')">
                ...
                <div class="card-body pt-0">
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_visits" style="width: 100%">
                        <thead>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isSuperAdminOrLeader) {
        ...
        fetch(`${apiUrl}/member-stats/visit`, {
    ...
    var datatable = $('#kt_table_visits').DataTable({
        ajax: {
            url: isSuperAdminOrLeader ? `${apiUrl}/manajemen-visit` : `${apiUrl}/manajemen-visit/user/` + currentUserId,
            type: 'GET',
            headers: { 'Authorization': `Bearer ${token}` },
            data: function(d) {
                if (selectedUserId) {
...
window.exportData = function(module) {
    ...
    let url = `{{ url('/api/export/manajemen-visit') }}?`;
    if (isSuperAdminOrLeader) {
    ...
    .then(blob => {
        ...
        a.href = downloadUrl;
        a.download = 'manajemen_visit.xlsx';
...
window.viewMemberData = function(userId, el) {
    ...
    $('#kt_table_visits').DataTable().ajax.url(`${apiUrl}/manajemen-visit/user/${userId}`).load();
}
