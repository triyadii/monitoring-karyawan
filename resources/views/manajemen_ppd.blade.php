...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var datatable = $('#kt_table_ppds').DataTable({
        ajax: {
            url: isSuperAdminOrLeader ? `${apiUrl}/manajemen-ppd` : `${apiUrl}/manajemen-ppd/user/` + currentUserId,
            type: 'GET',
            headers: { 'Authorization': `Bearer ${token}` },
            data: function(d) {
                if (selectedUserId) {
...
window.viewMemberData = function(userId, el) {
    ...
    $('#kt_table_ppds').DataTable().ajax.url(`${apiUrl}/manajemen-ppd/user/${userId}`).load();
}
...
window.showAllMembersData = function(el) {
    ...
    $('.member-card').removeClass('border-primary bg-light-primary');
    if (el) $(el).addClass('border-primary bg-light-primary');
    $('#kt_table_ppds').DataTable().ajax.url(`${apiUrl}/manajemen-ppd`).load();
}
});
</script>
@endsection
