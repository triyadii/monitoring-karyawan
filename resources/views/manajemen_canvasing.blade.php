...
@section('title', 'Manajemen Canvasing')
...
<!-- Modal Add/Edit Canvasing -->
<div class="modal fade" id="kt_modal_add_canvasing" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header" id="kt_modal_add_canvasing_header">
                <h2 class="fw-bold" id="modal_title">Tambah Canvasing</h2>
            ...
            <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                <form id="kt_modal_add_canvasing_form" class="form" action="#">
                    <input type="hidden" id="canvasing_uuid" name="uuid" value="" />
...
<!-- Modal Detail Canvasing -->
<div class="modal fade" id="kt_modal_detail_canvasing" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Detail Canvasing</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal">
                    <i class="ki-duotone ki-cross fs-1">
                        <span class="path1"></span>
                        <span class="path2"></span>
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            { data: 'uuid', orderable: false, render: function(data, type, row) {
                ...
                if (hasCrudAccess) {
                    actions += `
                        ...
                        <button class="btn btn-icon btn-sm btn-light-danger" onclick="deleteCanvasing('${data}')" title="Delete">
    ...
    $('[data-kt-canvasing-table-filter="search"]').on('keyup', function () {
        datatable.search(this.value).draw();
    });

    // Handle Form Submit
    const form = document.getElementById('kt_modal_add_canvasing_form');
    form.addEventListener('submit', function (e) {
        e.preventDefault();

        const submitButton = document.getElementById('kt_modal_add_canvasing_submit');
        ...
        .then(response => {
            ...
            if (response.ok) {
                $('#kt_modal_add_canvasing').modal('hide');
...
window.resetForm = function() {
    ...
    document.getElementById('modal_title').innerText = "Tambah Canvasing";
}

window.editCanvasing = function(canvasing) {
    resetForm();
    document.getElementById('modal_title').innerText = "Edit Canvasing";
    document.getElementById('canvasing_uuid').value = canvasing.uuid;
    document.getElementById('namaClient').value = canvasing.namaClient;
...
window.deleteCanvasing = function(uuid) {
    Swal.fire({
        ...
        if (result.value) {
            ...
            fetch(`${apiUrl}/manajemen-canvasing/${uuid}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Accept': 'application/json'
...
window.showDetailCanvasing = function(canvasing) {
    document.getElementById('detail_nama_client').innerText = canvasing.namaClient || '-';
    document.getElementById('detail_nomor_telepon').innerText = canvasing.nomorTelepon || '-';
    document.getElementById('detail_alamat').innerText = canvasing.alamat || '-';
    
    let pembuatName = canvasing.user ? canvasing.user.nama : '-';
